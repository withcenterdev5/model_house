...
- Realtime Database 를 적극적으로 활용한다. 특히, 비용적인 문제로 채팅 방 목록과 채팅 메시지 목록을 Realtime Database 로 쓰고 싶지만, 대부분의 개발자들은 Realtime Database 의 사용을 꺼려한다. 물론 Firestore 가 활용도가 훨씬 높다. 그래서 사용자 관리, 채팅, 게시판 기타 등등에 Firestore 를 쓰지만 가능하면 Realtime Database 를 적극 활용한다. 예를 들면, 각종 로그나, 단순한 데이터 읽고 쓰기를 할 때에 특히 필요하다.
