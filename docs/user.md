...
관리자는 `/settings/admins` 문서에 둔다. 자세한 것은 [하우스 엔진]()을 참고한다.
...
누구든 `i.claimRoot()` 함수를 최초로 호출하면 root 관리자가 될 수 있다. 이것은 Security rules 에 의해서 보안 관리되며, 두번째 유저 부터는 permission denied 에러가 발생한다.
